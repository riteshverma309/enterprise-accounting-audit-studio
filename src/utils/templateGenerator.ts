...
Human Resources & People Ops, 110000.00`,
  ...
  },
  {
    ...
    title: 'Customer Directory Master & On-the-Fly Custom Attributes',
    description: 'Customer contact master records bulk importer with dynamic custom attribute columns (e.g. attr_flat_no, attr_carpet_area, attr_admission_date).',
    filename: 'customers_master_template.csv',
    category: 'Sub-Ledgers',
    headers: ['code', 'name', 'email', 'phone', 'billingAddress', 'category', 'taxId', 'paymentTermsDays', 'notes', 'attr_carpet_area_sqft', 'attr_unit_flat_no', 'attr_is_tax_exempt'],
    sampleCsv: (_tenantName = 'Acme Corp', _currency = 'USD') => `code, name, email, phone, billingAddress, category, taxId, paymentTermsDays, notes, attr_carpet_area_sqft, attr_unit_flat_no, attr_is_tax_exempt
CUST-001, Acme Global Technologies Inc, finance@acmeglobal.com, +1 (555) 234-5678, "100 Silicon Ave Suite 400 San Francisco CA", Enterprise Client, US-EIN-94827104, 30, "Premier SaaS Client", 1850, 402, TRUE
CUST-002, Dr. Evelyn Reed (Flat B-502), evelyn.reed@residence.org, +1 (555) 876-5432, "Tower B Apt 502 Highgate Gardens", Housing Society Resident, TX-IND-48201, 15, "HOA Maintenance Monthly Resident", 1200, 502, FALSE
CUST-003, St. Jude Academy (Roll # 2026-98), admissions@stjude-edu.org, +1 (555) 345-6789, "450 University Way Cambridge MA", School Student, EDU-992019, 30, "Education Tuition & Transport Enrollment", 950, 101, TRUE`,
    sampleJson: () => JSON.stringify(
      [
        {
          code: 'CUST-001',
    ...
    fieldDocs: [
      { field: 'code', required: true, description: 'Unique customer identifier code (e.g. CUST-001)' },
      { field: 'name', required: true, description: 'Customer legal or individual display name' },
      { field: 'email', required: true, description: 'Contact email address for invoice delivery' },
      { field: 'phone', required: false, description: 'Primary phone contact' },
      { field: 'billingAddress', required: false, description: 'Street, city, postal billing address' },
      { field: 'category', required: false, description: 'Customer classification (e.g., Enterprise Client, Housing Society Resident)' },
      { field: 'taxId', required: false, description: 'Corporate Tax Identification / SSN / GSTIN' },
      { field: 'paymentTermsDays', required: false, description: 'Credit payment window in days (e.g. 15, 30, 45)' },
      { field: 'notes', required: false, description: 'Internal account remarks' },
      { field: 'attr_<key>', required: false, description: 'Dynamic custom attribute columns matching defined schema fields (e.g., attr_flat_no, attr_admission_date)' },
    ],
  },
  {
...
2026-08-13, BANK-DEP-992, Direct Wire Deposit - Managed Advisory Services, 18500.00, Nexus Logistics EU B.V., 1010`,
  ...
  {
    id: 'customer_opening_balances',
    title: 'Customer FY Opening Balances (Pre-Populated Master)',
    description: 'Pre-populated template with all registered customers. Update only the Opening Balance Amount and CreditDebitFlag (DR for customer owes / CR for overpayment & advance credit).',
    filename: 'customer_fy_opening_balances_template.csv',
    category: 'Sub-Ledgers',
    headers: [
      'CustomerCode',
      'CustomerName',
    ...
    sampleCsv: (_tenantName = 'Acme Corp', _currency = 'USD') => `CustomerCode, CustomerName, Category, FiscalYear, AsOfDate, OriginalInvoiceNumber, DueDate, OpeningBalanceAmount, CreditDebitFlag, OffsetAccountCode, Notes
CUST-001, Acme Global Technologies Inc, Enterprise Client, FY 2026-2027, 2026-04-01, OPN-CUST-001, 2026-04-30, 15000.00, DR, 3010, Prior FY unpaid SaaS subscription carryforward
CUST-002, Dr. Evelyn Reed (Flat B-502), Housing Society Resident, FY 2026-2027, 2026-04-01, OPN-CUST-002, 2026-04-30, 2400.00, DR, 3010, Maintenance fee arrears from FY 2025-2026
CUST-003, St. Jude Academy (Roll # 2026-98), School Student, FY 2026-2027, 2026-04-01, OPN-CUST-003, 2026-04-30, 1200.00, CR, 3010, Customer overpayment / Advance tuition deposit carried forward`,
    sampleJson: () => JSON.stringify(
      [
        {
          ...
          asOfDate: '2026-04-01',
          originalInvoiceNumber: 'OPN-CUST-001',
          dueDate: '2026-04-30',
          openingBalanceAmount: 15000.0,
          creditDebitFlag: 'DR',
          offsetAccountCode: '3010',
          notes: 'Prior FY unpaid SaaS subscription carryforward',
        },
        {
          customerCode: 'CUST-003',
          customerName: 'St. Jude Academy',
          category: 'School Student',
          fiscalYear: 'FY 2026-2027',
          asOfDate: '2026-04-01',
          originalInvoiceNumber: 'OPN-CUST-003',
          dueDate: '2026-04-30',
          openingBalanceAmount: 1200.0,
          creditDebitFlag: 'CR',
          offsetAccountCode: '3010',
          notes: 'Customer overpayment / Advance deposit carried forward',
        },
      ],
      null,
      2
    ...
    fieldDocs: [
      { field: 'CustomerCode', required: true, description: 'Master customer code (e.g. CUST-001). Pre-populated from customer master.' },
      { field: 'CustomerName', required: true, description: 'Customer legal or individual display name. Pre-populated.' },
      { field: 'Category', required: false, description: 'Customer category/industry group.' },
      { field: 'FiscalYear', required: true, description: 'Target financial year (e.g. FY 2026-2027)' },
      { field: 'AsOfDate', required: true, description: 'Fiscal cutover effective date (YYYY-MM-DD, e.g. 2026-04-01)' },
      { field: 'OriginalInvoiceNumber', required: false, description: 'Historical invoice or opening reference number (e.g. OPN-CUST-001)' },
      { field: 'DueDate', required: true, description: 'Receivable due date for aging calculation (YYYY-MM-DD)' },
      { field: 'OpeningBalanceAmount', required: true, description: 'REQUIRED TO UPDATE: Outstanding amount. Positive numbers. Rows with 0 or blank are safely skipped.' },
      { field: 'CreditDebitFlag', required: true, description: 'REQUIRED TO UPDATE: DR (Customer owes / Receivable) or CR (Customer overpaid / Advance Credit balance). Negative amounts are automatically treated as CR.' },
      { field: 'OffsetAccountCode', required: false, description: 'GL balancing equity account code (defaults to 3010 Opening Balance Equity)' },
      { field: 'Notes', required: false, description: 'Audit trail and ledger memo remarks' },
    ],
  },
];
