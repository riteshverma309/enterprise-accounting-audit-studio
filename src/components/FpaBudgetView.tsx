    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 pb-4 border-b border-slate-800">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-xl font-black text-white tracking-tight">FP&A Budget vs. Actual Variance Analysis</h1>
          ...
          <p className="text-xs text-slate-400 mt-1">
            Departmental financial planning, real-time general ledger expense tracking, and variance burn indicators.
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          <div className="px-4 py-2 bg-slate-900 border border-slate-800 rounded-xl flex items-center gap-3">
            ...
            <div>
              <div className="text-[10px] text-slate-400 uppercase tracking-wider font-semibold">Total Corporate Budget</div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-slate-900 border border-slate-800 p-4 rounded-2xl space-y-1">
          <span className="text-xs font-semibold text-slate-400">Total YTD General Ledger Spend</span>
          <div className="text-2xl font-black text-indigo-400 font-mono">
            {currencySymbol}{totalYtdActual.toLocaleString()}
          </div>
          <p className="text-[11px] text-slate-500">P&L Expense Accounts Aggregate</p>
        </div>

        <div className="bg-slate-900 border border-slate-800 p-4 rounded-2xl space-y-1">
          <span className="text-xs font-semibold text-slate-400">Net Unused Remaining Allocation</span>
          ...
          <p className="text-[11px] text-slate-500">Available Cap for Fiscal Year</p>
        </div>

        <div className="bg-slate-900 border border-slate-800 p-4 rounded-2xl space-y-1">
          <span className="text-xs font-semibold text-slate-400">Overall Budget Burn Rate</span>
          <div className="text-2xl font-black text-purple-400 font-mono">
            {Math.round((totalYtdActual / totalAnnualBudget) * 100)}%
          </div>
          <p className="text-[11px] text-slate-500">Target Benchmark: &lt; 75% for Q3</p>
        </div>
      </div>
      ...
      <div className="bg-slate-900 rounded-2xl border border-slate-800 p-6 space-y-4">
        <div className="flex items-center justify-between pb-3 border-b border-slate-800">
          <h2 className="text-base font-bold text-white flex items-center gap-2">
            <Layers className="w-5 h-5 text-purple-400" /> Department Cost-Center Allocations
          </h2>
          <span className="text-xs text-slate-400 font-mono">Live Sync with General Ledger</span>
        </div>
              ...
              >
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                  ...
                  {selectedBudgetId === b.id ? (
                    ...
                    >
                      <Edit3 className="w-3.5 h-3.5" /> Adjust Budget
                    </button>
                  )}
                </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 pt-2 border-t border-slate-900 text-xs font-mono">
                  <div>
                    <span className="text-slate-500 text-[10px] uppercase">Annual Allocation</span>
                    <div className="font-bold text-slate-200">{currencySymbol}{b.annualBudget.toLocaleString()}</div>
                  </div>
                  <div>
                    <span className="text-slate-500 text-[10px] uppercase">YTD Actual Spend</span>
                  ...
                  <div>
                    <span className="text-slate-500 text-[10px] uppercase">Headroom (%)</span>
