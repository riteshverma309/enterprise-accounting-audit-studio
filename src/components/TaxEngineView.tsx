      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 pb-4 border-b border-slate-800">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-xl font-black text-white tracking-tight">{tr('Statutory Tax & Multi-Jurisdiction Engine')}</h1>
            <span className="px-2.5 py-0.5 bg-indigo-500/10 text-indigo-400 border border-indigo-500/30 rounded-full text-xs font-semibold">{tr('Global Tax Provisioning')}</span>
          </div>
          <p className="text-xs text-slate-400 mt-1">{tr('Automated sales tax, VAT, and GST accrued liabilities, rate matrices, and settlement voucher creation.')}</p>
        </div>
        ...
        <div className="flex items-center gap-3">
          <div className="px-4 py-2 bg-slate-900 border border-slate-800 rounded-xl flex items-center gap-3">
            ...
            <div>
              <div className="text-[10px] text-slate-400 uppercase tracking-wider font-semibold">{tr('Total Accrued Tax Liability')}</div>
      ...
      <div className="bg-slate-900 rounded-2xl border border-slate-800 p-6 space-y-4">
        <div className="flex items-center justify-between pb-3 border-b border-slate-800">
          <h2 className="text-base font-bold text-white flex items-center gap-2">
            <Globe className="w-5 h-5 text-indigo-400" />{tr('Active Statutory Tax Rate Matrix & Accrued Balances')}</h2>
          <span className="text-xs text-slate-400 font-mono">{tr('Automatic General Ledger Postings')}</span>
        </div>
          ...
          {taxJurisdictions.map((jur) => (
            ...
            >
              ...
              <div className="flex items-center justify-between pt-1">
                <div>
                  <span className="text-[10px] text-slate-500 uppercase tracking-wider font-semibold">{tr('Accrued Tax Liability')}</span>
                ...
                >
                  <Send className="w-3.5 h-3.5" />{tr('Post Tax Settlement')}</button>
              </div>
            </div>
          ))}
        </div>
