    ...
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-xl font-bold text-white">Immutable System Security & Scope Audit Log</h1>
          ...
          <p className="text-xs text-slate-400 mt-1">
            Real-time audit record of API request context headers, scope enforcement, role permissions, and journal postings.
          </p>
        </div>
      </div>
      ...
      <div className="bg-slate-900 rounded-2xl border border-slate-800 p-5 space-y-3">
        <div className="flex items-center gap-2 text-xs font-bold text-indigo-400">
          ...
          <span>Active Request Context Headers Inspection</span>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 font-mono text-xs">
          <div className="p-3 bg-slate-950 rounded-xl border border-slate-800">
            <span className="text-[10px] text-slate-500 uppercase font-bold">x-tenant-id</span>
            <p className="text-indigo-300 font-bold truncate mt-1">{activeTenant.id}</p>
          </div>
          <div className="p-3 bg-slate-950 rounded-xl border border-slate-800">
            <span className="text-[10px] text-slate-500 uppercase font-bold">x-organization-id</span>
            <p className="text-indigo-300 font-bold truncate mt-1">{activeOrganization?.id || 'GLOBAL_ALL'}</p>
          </div>
          <div className="p-3 bg-slate-950 rounded-xl border border-slate-800">
            <span className="text-[10px] text-slate-500 uppercase font-bold">x-branch-id</span>
            <p className="text-indigo-300 font-bold truncate mt-1">{activeBranch?.id || 'HQ_BRANCH'}</p>
          </div>
          <div className="p-3 bg-slate-950 rounded-xl border border-slate-800">
            <span className="text-[10px] text-slate-500 uppercase font-bold">Active Auth Token Role</span>
      ...
      <div className="bg-slate-900 rounded-2xl border border-slate-800 shadow-sm overflow-hidden p-6 space-y-4">
        <h3 className="text-sm font-bold text-white">System Security Events Feed</h3>
