    ...
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      ...
      <div>
        <h1 className="text-xl font-bold text-slate-100 flex items-center gap-2">
          <Globe2 className="w-6 h-6 text-blue-400" />{tr('Global Multi-Tenant Corporate Consolidation & FX Translation')}</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-slate-900 border border-slate-800 p-4 rounded-xl">
          <div className="flex items-center justify-between text-slate-400 mb-1">
            <span className="text-xs font-semibold">{tr('Gross Consolidated Revenue')}</span>
        ...
        <div className="bg-slate-900 border border-slate-800 p-4 rounded-xl">
          <div className="flex items-center justify-between text-slate-400 mb-1">
            <span className="text-xs font-semibold">{tr('Intercompany Eliminations')}</span>
            <ArrowRightLeft className="w-4 h-4 text-amber-400" />
          </div>
          <div className="text-2xl font-bold text-amber-400 font-mono">
            - {consolidatedFinancials.presentationCurrency} {elimAmount.toLocaleString()}
          </div>
          <p className="text-[11px] text-slate-500 mt-1">{tr('Elimination of intercompany sales & service fees')}</p>
        </div>

        <div className="bg-slate-900 border border-slate-800 p-4 rounded-xl">
          <div className="flex items-center justify-between text-slate-400 mb-1">
            <span className="text-xs font-semibold">{tr('Net Consolidated Group Revenue')}</span>
            <CheckCircle2 className="w-4 h-4 text-emerald-400" />
          </div>
          <div className="text-2xl font-bold text-emerald-400 font-mono">
            {consolidatedFinancials.presentationCurrency} {netRev.toLocaleString()}
          </div>
          <p className="text-[11px] text-slate-500 mt-1">{tr('Final SEC 10-K / IFRS consolidated income statement')}</p>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between p-4 bg-slate-900 border border-slate-800 rounded-xl">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-xs font-bold text-slate-200">{tr('Intercompany Balance & Revenue Eliminations')}</h3>
            <p className="text-[11px] text-slate-400">{tr('Eliminate double-counted intercompany receivables, payables, and cross-entity licensing revenue.')}</p>
          </div>
        </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden">
        <div className="p-4 border-b border-slate-800 flex items-center justify-between">
          <h2 className="text-sm font-semibold text-slate-200">{tr('Operating Entity Consolidation Ledger')}</h2>
          <span className="text-xs text-slate-400 font-mono">{tr('IAS 21 Foreign Currency Translation Engine')}</span>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs text-slate-300">
            <thead className="bg-slate-950 text-slate-400 uppercase font-mono text-[10px] tracking-wider">
              <tr>
                <th className="p-3">{tr('Entity Name')}</th>
                <th className="p-3">{tr('Local Currency')}</th>
                <th className="p-3 text-right">{tr('Local Revenue')}</th>
            ...
            <tbody className="divide-y divide-slate-800 font-mono">
              ...
              <tr className="bg-slate-950/80 font-bold">
                <td colSpan={4} className="p-3 text-slate-300 font-sans">{tr('Gross Subtotal before Eliminations')}</td>
              ...
              {applyEliminations && (
                <tr className="bg-amber-950/20 text-amber-300 font-semibold">
                  <td colSpan={4} className="p-3 font-sans">{tr('Intercompany Elimination Adjustments')}</td>
              ...
              <tr className="bg-slate-950 text-slate-100 font-bold border-t-2 border-slate-700">
                <td colSpan={4} className="p-3 font-sans text-sm">{tr('FINAL NET CONSOLIDATED GROUP TOTAL')}</td>
