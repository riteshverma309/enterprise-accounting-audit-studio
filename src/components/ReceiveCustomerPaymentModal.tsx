      ...
      >
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h2 className="text-xl font-bold tracking-tight text-white flex items-center gap-2">
                ...
                <span className="text-xs px-2.5 py-0.5 rounded-full bg-emerald-500/20 text-emerald-300 font-mono border border-emerald-400/30">{tr('AR Multi-Invoice Settlement')}</span>
              </h2>
              <p className="text-xs text-slate-400">
                {tr('Post customer funds received, apply across outstanding invoices or FY opening balances, and create double-entry GL ledger vouchers.')}
              </p>
            </div>
          </div>
          <button
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 bg-slate-50 p-4 rounded-xl border border-slate-200">
            {/* Customer Select */}
            <div>
              <label className="block text-xs font-semibold text-slate-700 mb-1">{tr('Customer Name')}<span className="text-red-500">*</span>
              </label>
              <select
              ...
              {selectedCustomer && (
                <div className="mt-1 text-xs text-slate-500 flex items-center justify-between">
                  ...
                  <span className="font-semibold text-slate-700">
                    {tr('Total Due:')} {activeTenant.currency} {totalCustomerOutstanding.toLocaleString(undefined, { minimumFractionDigits: 2 })}
                  </span>
                </div>
              )}
            </div>
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-700 mb-1">
                {tr('Total Amount Received')} ({activeTenant.currency}) <span className="text-red-500">*</span>
              </label>
              ...
              <div className="mt-1 flex items-center justify-between">
                ...
                >
                  {tr('Pay All Outstanding')} ({totalCustomerOutstanding.toFixed(2)})
                </button>
              </div>
            </div>
            ...
            {/* Payment Date */}
            <div>
              <label className="block text-xs font-semibold text-slate-700 mb-1">{tr('Payment Date')}<span className="text-red-500">*</span>
              </label>
              <input
            ...
            {/* Payment Method */}
            <div>
              <label className="block text-xs font-semibold text-slate-700 mb-1">{tr('Payment Method')}<span className="text-red-500">*</span>
              </label>
              <select
                ...
                className="w-full px-3 py-2 bg-white border border-slate-300 rounded-lg text-sm text-slate-800 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500"
              >
                <option value="BANK_TRANSFER">{tr('Bank Wire / Electronic Transfer')}</option>
                <option value="ACH">{tr('Direct ACH Debit / Direct Deposit')}</option>
                <option value="CHECK">{tr('Paper Check / Cheque Voucher')}</option>
                <option value="CREDIT_CARD">{tr('Credit Card / Gateway Settlement')}</option>
                <option value="UPI">{tr('UPI / Instant QR Payment')}</option>
                <option value="CASH">{tr('Cash Over Counter')}</option>
                <option value="OTHER">{tr('Other / Clearing House')}</option>
              </select>
            </div>

            {/* Receiving Bank Account */}
            <div>
              <label className="block text-xs font-semibold text-slate-700 mb-1">{tr('Deposit to Account (GL Debit)')}<span className="text-red-500">*</span>
              </label>
              <select
            ...
            {/* Reference / Check # */}
            <div>
              <label className="block text-xs font-semibold text-slate-700 mb-1">{tr('Reference / Check / UTR Number')}</label>
              <input
                id="payment-reference-input"
                type="text"
                value={referenceNumber}
                onChange={(e) => setReferenceNumber(e.target.value)}
                placeholder={tr('e.g. WIRE-9941, CHK-1082')}
                className="w-full px-3 py-2 bg-white border border-slate-300 rounded-lg text-sm text-slate-800 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500"
              />
            </div>
          </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2">
                <h3 className="text-sm font-bold text-slate-900 flex items-center gap-1.5">
                  <Receipt className="w-4 h-4 text-emerald-600" />{tr('Invoice Settlement Allocations')}</h3>
                <span className="text-xs px-2 py-0.5 rounded-full bg-slate-100 text-slate-600 font-medium">
                  {allocations.length} {tr('Unpaid / Open Invoices')}
                </span>
              </div>
              ...
              {allocations.length > 0 && (
                <div className="flex items-center gap-2">
                  ...
                  >
                    <Sparkles className="w-3.5 h-3.5" />{tr('Auto-Allocate Oldest (FIFO)')}</button>
                  <button
                    type="button"
                    onClick={() => {
            ...
            {allocations.length === 0 ? (
              <div className="p-8 text-center bg-slate-50 rounded-xl border border-dashed border-slate-300 text-slate-500">
                <p className="text-sm font-medium">{tr('No open invoices or opening balances found for this customer.')}</p>
              ...
              <div className="border border-slate-200 rounded-xl overflow-hidden shadow-2xs">
                <table className="w-full text-left text-xs border-collapse">
                  <thead className="bg-slate-100 text-slate-600 font-semibold border-b border-slate-200">
                    <tr>
                      <th className="p-3">{tr('Invoice Details')}</th>
                      <th className="p-3">{tr('Due Date')}</th>
                      <th className="p-3 text-right">{tr('Original Amount')}</th>
                      <th className="p-3 text-right">{tr('Already Paid')}</th>
                      <th className="p-3 text-right">{tr('Remaining Due')}</th>
                      <th className="p-3 w-32 text-right">{tr('Payment Allocated')}</th>
                      <th className="p-3 w-28 text-right">{tr('Discount / Write-off')}</th>
                      <th className="p-3 text-center w-24">{tr('Action')}</th>
                    </tr>
                  </thead>
                      ...
                      return (
                        <tr key={row.invoiceId} className={`hover:bg-slate-50/80 transition ${isFull ? 'bg-emerald-50/30' : ''}`}>
                          <td className="p-3">
                            ...
                            <div className="text-[11px] text-slate-400">{tr('Issued:')} {row.issueDate}</div>
                          </td>
                          ...
                          <td className="p-3">
                            ...
                            {isPastDue && (
                              <span className="text-[10px] text-rose-500 font-semibold">{tr('Overdue')}</span>
                            )}
                          </td>
                          ...
                          <td className="p-3 text-center">
                            ...
                            >
                              {isFull ? tr('Paid in Full') : tr('Pay Full')}
                            </button>
                          </td>
                        </tr>
                      );
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="p-4 bg-slate-900 text-white rounded-xl border border-slate-800 space-y-2.5">
              <div className="text-xs font-bold text-slate-300 uppercase tracking-wider">{tr('Settlement Financial Summary')}</div>
              <div className="flex justify-between text-xs text-slate-300">
                <span>{tr("Total Received from Customer:")}</span>
                <span className="font-bold text-white">{activeTenant.currency} {totalAmountReceived.toFixed(2)}</span>
              </div>
              <div className="flex justify-between text-xs text-emerald-400">
                <span>{tr("Total Applied to Invoices:")}</span>
                <span className="font-bold">{activeTenant.currency} {totalAllocated.toFixed(2)}</span>
              </div>
              {totalDiscount > 0 && (
                <div className="flex justify-between text-xs text-amber-400">
                  <span>{tr("Early Settlement Discounts Allowed:")}</span>
              ...
              <div className="pt-2 border-t border-slate-800 flex justify-between text-xs">
                <span className="text-slate-300">{tr("Unallocated Advance / Credit Balance:")}</span>
            ...
            <div className="p-4 bg-slate-50 rounded-xl border border-slate-200 text-xs space-y-2">
              <div className="font-bold text-slate-800 flex items-center gap-1">
                <ShieldCheck className="w-3.5 h-3.5 text-emerald-600" />{tr('Automatic GL Journal Voucher Impact')}</div>
          ...
          {/* Notes */}
          <div>
            <label className="block text-xs font-semibold text-slate-700 mb-1">{tr('Internal Settlement Notes & Remittance Details')}</label>
            <input
              id="payment-notes-input"
              type="text"
              value={notes}
              onChange={(e) => setNotes(e.target.value)}
              placeholder={tr('e.g. Wire transfer confirmed via JPMorgan Treasury Portal. Batch #8812.')}
              className="w-full px-3 py-2 bg-white border border-slate-300 rounded-lg text-sm text-slate-800 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500"
            />
          </div>
        </form>
        ...
        <div className="p-4 bg-slate-100 border-t border-slate-200 flex items-center justify-between">
          <button
            type="button"
            onClick={onClose}
            className="px-4 py-2 text-sm font-medium text-slate-600 hover:text-slate-800 hover:bg-slate-200 rounded-xl transition cursor-pointer"
          >
            {tr('Cancel')}
          </button>
          ...
          <button
            ...
            className="px-6 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-bold rounded-xl shadow-md hover:shadow-lg transition flex items-center gap-2 cursor-pointer"
          >
            <CheckCircle2 className="w-4 h-4" />
            {tr('Post Payment Receipt & Update Ledger')}
          </button>
        </div>
      </div>
    </div>
